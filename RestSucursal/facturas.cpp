...
void Facturas :: mostrar(){
    ...
    ui->label_fecha->setText("2020-07-26");
    ui->label_hora->setText("10:41:00");
    ui->label_impuesto->setText("10");
    ui->label_descuento->setText("10");
    ui->label_total->setText(QString::number(Saldo));
    ...
    if(mesas->getreservar() == "Reservar"){
        query.prepare("SET XACT_ABORT ON; SELECT c.codigo,p.nombre,p.apel_paterno,p.apel_materno FROM [192.168.0.104].Restaurante.dbo.Clientes c INNER JOIN [192.168.0.104].Restaurante.dbo.Persona p ON c.codigo = p.codigo");
        query.exec();
        while(query.next()){
...
void Facturas::on_pushButton_cancelar_clicked()
{
    if(ingresados == 0){
        ...
        if(mesas->getreservar() == "Reservar"){
            ...
            QSqlQuery select;
            QStringList Lista;
            select.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.104].Restaurante.dbo.Facturas");
            select.exec();
            while(select.next()){
            ...
            QSqlQuery select1;
            QStringList Lista1;
            select1.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.104].Restaurante.dbo.Cajeros");
            select1.exec();
            while(select1.next()){
            ...
            {
                QSqlQuery query;
                query.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.104].Restaurante.dbo.Facturas VALUES('"+cod_fact+"','"+a+"','"+b+"','"+descuento_+"','"+impuestos_+"','"+cajero+"','"+codigo+"')");
                if(query.exec()){
                    qDebug() <<"Datos ingresados en Facturas Principal";
                }
                else{
                    qDebug() <<"Datos no ingresados en Facturas Principal";
                }
            }
            //ui->label_nombre->setText(codigo);
            ...
            QSqlQuery selectf;
            QStringList Listaf;
            selectf.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.104].Restaurante.dbo.Lineas_Fact");
            selectf.exec();
            while(selectf.next()){
            ...
            QSqlQuery querylineas;
                ...
                QSqlQuery selectp;
                QStringList Listap;
                selectp.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.104].Restaurante.dbo.Pedidos");
                selectp.exec();
                while(selectp.next()){
                ...
                selectc.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.104].Restaurante.dbo.Categorias c INNER JOIN  [192.168.0.104].Restaurante.dbo.Producto ipp ON c.cod_catg = ipp.cod_catg INNER JOIN Producto p ON ipp.enlace = p.enlace");
                selectc.exec();
                while(selectc.next()){
                ...
                for(int p = 0;p < nombre.size(); p++){
                    ...
                    if(encontro){
                        ...
                        querype.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.104].Restaurante.dbo.Pedidos VALUES('"+cod_pro_+"','"+Listan[index]+"','"+Listaprecio[index]+"','"+Listac[index]+"')");
                        QString precio__=precio[p];
                        querylineas.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.104].Restaurante.dbo.Lineas_Fact VALUES('"+cod_fact+"','"+cod_lineas_+"','"+cantidad[p]+"','"+precio__+"','"+cod_pro_+"')");
                        if(querype.exec() && querylineas.exec())
                            qDebug() << "Datos ingresados a Pedidos y ha Lineas_Fact principal";
                        else
                            qDebug() << "Datos no ingresados a Pedidos principal";
                ...
                selectmesa.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.104].Restaurante.dbo.Mesas");
                selectmesa.exec();
                ...
                if(estamesa){
                    ...
                    qDebug() <<"eliminando mesa e insertando principal";
                    querymesa.prepare("SET XACT_ABORT ON; DELETE [192.168.0.104].Restaurante.dbo.Mesas WHERE cod_mesa ="+mesa_);
                    querymesa.exec();
                    querymesa2.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.104].Restaurante.dbo.Mesas VALUES ('"+mesa_+"','"+codigo+"','"+recuperar+"')");
                    querymesa2.exec();
                    ...
                    msg.setWindowTitle("ERROR EN REGISTRAR");
                    msg.setText("La Factura Principal no fue registrada");
                }
            ...
            QSqlQuery querylineas;
                ...
                QSqlQuery selectc,querype;
                QStringList Listac,Listan,Listaprecio;
                selectc.prepare("SELECT * FROM [192.168.0.104].Restaurante.dbo.Categorias c INNER JOIN  [192.168.0.104].Restaurante.dbo.Producto ipp ON c.cod_catg = ipp.cod_catg INNER JOIN Producto p ON ipp.enlace = p.enlace");
                selectc.exec();
                while(selectc.next()){
                    Listac << selectc.value(1).toByteArray().constData();
                    Listan << selectc.value(3).toByteArray().constData();
                    Listaprecio << selectc.value(7).toByteArray().constData();
                }
                //qDebug() << Listac <<Listan << Listaprecio;
                ...
                if(estamesa){
                    ...
                    msg.setIcon(QMessageBox::Warning);
                    msg.setWindowTitle("ERROR EN REGISTRAR");
                    msg.setText("La Factura no fue registrada");
                }
