...
void Facturas::on_pushButton_cancelar_clicked()
{
    ...
    QSqlQuery querylineas;
        ...
        selectc.prepare("SELECT * FROM [192.168.0.103].Restaurante.dbo.Categorias c INNER JOIN  [192.168.0.103].Restaurante.dbo.Producto ipp ON c.cod_catg = ipp.cod_catg INNER JOIN Producto p ON ipp.enlace = p.enlace");
        selectc.exec();
        while(selectc.next()){
