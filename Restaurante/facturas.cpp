...
void Facturas :: mostrar(){
    ...
    mesas = new Mesas();
    if(mesas->getreserva() == "Reservar"){

        query.prepare("SET XACT_ABORT ON; SELECT c.codigo,p.nombre,p.apel_paterno,p.apel_materno FROM [192.168.0.102].Restaurante.dbo.Clientes c INNER JOIN [192.168.0.102].Restaurante.dbo.Persona p ON c.codigo = p.codigo");
        query.exec();
        while(query.next()){
...
void Facturas::on_pushButton_cancelar_clicked()
{
    if(ingresados == 0){
        ...
        mesas = new Mesas();
        if(mesas->getreserva() == "Reservar" ){
            qDebug() << mesas->getreserva()<<"HOLALLA";
            //viendo facturas
            QSqlQuery select;
            QStringList Lista;
            select.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.102].Restaurante.dbo.Facturas");
            select.exec();
            while(select.next()){
            ...
            QSqlQuery select1;
            QStringList Lista1;
            select1.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.102].Restaurante.dbo.Cajeros");
            select1.exec();
            while(select1.next()){
            ...
            {
                QSqlQuery query;
                query.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.102].Restaurante.dbo.Facturas VALUES('"+cod_fact+"','"+a+"','"+b+"','"+descuento_+"','"+impuestos_+"','"+cajero+"','"+codigo+"')");
                if(query.exec()){
                    qDebug() <<"Datos ingresados en Facturas Sucursal";
                }
                else{
                    qDebug() <<"Datos no ingresados en Facturas Sucursal";
                }
            }
            ...
            QSqlQuery selectf;
            QStringList Listaf;
            selectf.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.102].Restaurante.dbo.Lineas_Fact");
            selectf.exec();
            while(selectf.next()){
            ...
            QSqlQuery selectp;
            QStringList Listap;
            selectp.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.102].Restaurante.dbo.Pedidos");
            selectp.exec();
            while(selectp.next()){
            ...
            selectc.prepare("SET XACT_ABORT ON; SELECT * FROM Categorias c INNER JOIN Producto p ON c.cod_catg = p.cod_catg INNER JOIN [192.168.0.102].Restaurante.dbo.Producto ipp ON p.enlace = ipp.enlace");
            selectc.exec();
            while(selectc.next()){
                Listac << selectc.value(1).toByteArray().constData();//cambio por el enlace
            ...
            for(int p = 0;p < nombre.size(); p++){
                ...
                if(encontro){
                    ...
                    querype.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.102].Restaurante.dbo.Pedidos VALUES('"+cod_pro_+"','"+Listan[index]+"','"+Listaprecio[index]+"','"+Listac[index]+"')");
                    QString precio__= precio[p];
                    querylineas.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.102].Restaurante.dbo.Lineas_Fact VALUES('"+cod_fact+"','"+cod_lineas_+"','"+cantidad[p]+"','"+precio__+"','"+cod_pro_+"')");
                    if(querype.exec() && querylineas.exec())
                        qDebug() << "Datos ingresados a Pedidos y ha Lineas_Fact Sucursales";
                    else
                        qDebug() << "Datos no ingresados a Pedidos Sucursales";
            ...
            selectmesa.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.102].Restaurante.dbo.Mesas");
            selectmesa.exec();
            ...
            if(estamesa){
                QSqlDatabase::database().commit();
                    qDebug() <<"eliminando mesa e insertando sucursal";
                    querymesa.prepare("SET XACT_ABORT ON; DELETE [192.168.0.102].Restaurante.dbo.Mesas WHERE cod_mesa ="+mesa_);
                    querymesa.exec();
                    qDebug() << codigo;
                    querymesa2.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.102].Restaurante.dbo.Mesas VALUES ('"+mesa_+"','"+codigo+"','"+recuperar+"')");
                    querymesa2.exec();
                ...
                msg.setText("La Factura Sucursal no fue registrada");
            }
            ...
            QSqlQuery selectc,querype;
            QStringList Listac,Listan,Listaprecio;
            selectc.prepare("SELECT * FROM Categorias c INNER JOIN Producto p ON c.cod_catg = p.cod_catg INNER JOIN [192.168.0.102].Restaurante.dbo.Producto ipp ON p.enlace = ipp.enlace");
            selectc.exec();
            while(selectc.next()){
