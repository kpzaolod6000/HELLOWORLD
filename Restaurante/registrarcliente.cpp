...
void RegistrarCliente::on_pushButton_guardar_clicked()
{
    ...
    if(mesas->getreserva() == "Reservar"){
        ...
        QSqlQuery select;
        QStringList Lista;
        select.prepare("SET XACT_ABORT ON; SELECT * FROM [192.168.0.102].Restaurante.dbo.Clientes");
        select.exec();
        while(select.next()){
            Lista << select.value(0).toByteArray().constData();
        }
        qDebug() << Lista << "RESERVA";
        ...
        if(esta == false){
            ...
            if(!nombre.isEmpty() && !apel_materno.isEmpty() && !apel_paterno.isEmpty() ){
                query.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.102].Restaurante.dbo.Persona VALUES('"+codigo+"','"+nombre+"','"+apel_paterno+"','"+apel_materno+"','"+direccion+"','"+telefonos+"','"+lugar+"')");
                query1.prepare("SET XACT_ABORT ON; INSERT INTO [192.168.0.102].Restaurante.dbo.Clientes VALUES ('"+codigo+"')");
