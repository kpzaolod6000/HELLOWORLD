...
//para guardar algunos arreglos moificados
static int Lista[30];
static int i = 0;
...
void RegistrarMesa::on_pushButton_helados_clicked()
{
    ...
    //ACA ELIMINAR SI O SI LAS FILAS VACIAS

    QSqlQuery query;

    query.prepare("SELECT nombres,precio,stock FROM Producto p INNER JOIN [192.168.0.100].Restaurante.dbo.Producto ipp ON p.enlace = ipp.enlace WHERE p.cod_catg = 5550");
    if(query.exec()){
...
void RegistrarMesa::on_pushButton_fria_clicked()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT nombres,precio,stock FROM Producto p INNER JOIN [192.168.0.100].Restaurante.dbo.Producto ipp ON p.enlace = ipp.enlace WHERE p.cod_catg = 55511");
    if(query.exec()){
        qDebug()<<"CONSULTA CORRECTA";
        int fila = 0;
        ui->tableWidget_productos->setColumnCount(3);
...
void RegistrarMesa::mostrarpedidos(QString nombre,QString precio)
{
    ...
    fila++;
    //ojo aca es numero de platillos
...
void RegistrarMesa::on_pushButton_quitar_clicked()
{
    ...
    if(setfila != -1){
        ...
        Lista[setfila] = -1;//aca lo que elimine se elimina en la lista de pedidos repetidos
    }else{
        ui->label_puesto->setText("Por favor escoja el plato que quiere eliminar");
    }
    //cuando se quita uno
