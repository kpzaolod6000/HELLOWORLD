...
void Reportes::on_pushButton_clisucursal_clicked()
{
    ...
    query.prepare("SELECT * FROM Persona p INNER JOIN Clientes c ON p.codigo = c.codigo UNION SELECT * FROM [192.168.0.100].Restaurante.dbo.Persona p INNER JOIN [192.168.0.100].Restaurante.dbo.Clientes c ON p.codigo = c.codigo");
    if(query.exec()){
        qDebug()<<"CONSULTA CORRECTA";
        int fila = 0;
        ui->tableWidget->setColumnCount(7);
